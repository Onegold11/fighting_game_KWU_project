...
public class EnemyPlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if(Input.GetKeyUp(KeyCode.UpArrow) && playerRidibody.velocity.y > 0)
        {
            playerRidibody.velocity *= 0.5f;
        }
        //이동
        isWalked = false;
        if (Input.GetKey(KeyCode.LeftArrow) && playerRidibody.velocity.x > -2.0f)
        {
            Debug.Log("뒤로");
            if (isGrounded)
            {
        ...
        else if(Input.GetKey(KeyCode.RightArrow) && playerRidibody.velocity.x < 2.0f)
        {
            Debug.Log("앞으로");
            if (isGrounded)
            {
